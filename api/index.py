...
- Seedha URL Telegram ke sendPhoto me dete hain
- Telegram apne servers se image fetch karta hai
- Vercel sirf ek short API call karta hai — no timeout!
"""
...
def tg_send_photo_url(chat_id: int, image_url: str, caption: str = "") -> bool:
    """
    Telegram ko seedha URL dete hain.
    Telegram apne servers se image fetch karta hai — Vercel beech me nahi!
    Ye call sirf ~1-2 second leti hai.
    """
    try:
        r = requests.post(
...
def generate_image(prompt: str, chat_id: int) -> None:
    """
    Pollinations.ai se image generate karo.

    Vercel timeout fix:
    - Hum image DOWNLOAD nahi karte Vercel me
    - Seedha Pollinations URL Telegram ko dete hain (sendPhoto)
    - Telegram khud Pollinations se image fetch karta hai
    - Vercel ka kaam sirf URL banana + sendPhoto call = ~2 seconds only!
    """
    if not prompt.strip():
        tg_send(chat_id,
    ...
    # Pollinations image URL — ye immediately accessible hai
    # Telegram in servers se image fetch kar leta hai khud
    encoded   = urllib.parse.quote(prompt.strip())
    image_url = (
        f"https://image.pollinations.ai/prompt/{encoded}"
        f"?width=1024&height=1024&model=flux&nologo=true&enhance=true"
        f"&seed={abs(hash(prompt)) % 9999999}"
    )

    logger.info("Sending Pollinations URL to Telegram: %s", image_url[:80])

    success = tg_send_photo_url(
        chat_id,
        image_url,
    ...
    if not success:
        # Fallback: turbo model try karo (faster, lower quality)
        logger.info("Trying fallback with turbo model...")
        image_url_turbo = (
            f"https://image.pollinations.ai/prompt/{encoded}"
            f"?width=512&height=512&model=turbo&nologo=true"
        )
        success2 = tg_send_photo_url(
            chat_id,
            image_url_turbo,
            caption=f"🖼 *{prompt}*\n_Generated by DexMind × Pollinations.ai_ ✨"
        )
        if not success2:
            tg_send(chat_id,
                f"⚠️ Image generation failed. Try again!\n`/image {prompt}`"
            )
...
def get_weather(text: str) -> str:
    ...
    try:
        r = requests.get(
            f"https://wttr.in/{urllib.parse.quote(loc)}?format=3", timeout=6
        )
        r.raise_for_status()
